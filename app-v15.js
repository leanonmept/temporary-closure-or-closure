...
const FORM_RESOURCES = {
  '별지22': {
    title: '이동서비스일지',
    desc: '주·야간보호기관이 이동서비스를 제공한 경우 작성·보관하는 서식입니다.',
    current: 'https://www.suwonudc.co.kr/swjjc/base/board/read?boardManagementNo=36&boardNo=1931',
    archive2025: 'https://www.carefor.co.kr/cs/view_notice.php?calmgno=45793&rtnUrl=%2Fcs%2Flist_notice.php%3F'
  },
  '별지23': {
    title: '주·야간보호 목욕서비스 제공일지',
    desc: '주·야간보호기관이 목욕서비스를 제공한 경우 작성·비치하는 서식입니다.',
    current: 'https://www.suwonudc.co.kr/swjjc/base/board/read?boardManagementNo=36&boardNo=1931',
    archive2025: 'https://www.carefor.co.kr/cs/view_notice.php?calmgno=45793&rtnUrl=%2Fcs%2Flist_notice.php%3F'
  },
  '별지24': {
    title: '업무수행일지',
    desc: '연도와 적용시점에 따라 서식명과 작성대상이 달라집니다.',
    worklogPdf: 'https://www.carefor.co.kr/ct_att/contents_article/0/202601/46327/yp5Oefg7hD.pdf',
    current: 'https://www.suwonudc.co.kr/swjjc/base/board/read?boardManagementNo=36&boardNo=1931',
    archive2025: 'https://www.carefor.co.kr/cs/view_notice.php?calmgno=45793&rtnUrl=%2Fcs%2Flist_notice.php%3F'
  },
  '별지24-2': {
    title: '업무수행일지',
    desc: '2025년과 2026년의 서식명·작성대상이 다르므로 해당 기간 서식을 확인하세요.',
    worklogPdf: 'https://www.carefor.co.kr/ct_att/contents_article/0/202601/46327/yp5Oefg7hD.pdf',
    current: 'https://www.suwonudc.co.kr/swjjc/base/board/read?boardManagementNo=36&boardNo=1931',
    archive2025: 'https://www.carefor.co.kr/cs/view_notice.php?calmgno=45793&rtnUrl=%2Fcs%2Flist_notice.php%3F'
  },
  '별지24-3': {
    ...
    desc: '2026.1.1. 신설된 치매전담형 기관용 업무수행일지입니다.',
    worklogPdf: 'https://www.carefor.co.kr/ct_att/contents_article/0/202601/46327/yp5Oefg7hD.pdf',
    current: 'https://www.suwonudc.co.kr/swjjc/base/board/read?boardManagementNo=36&boardNo=1931'
  },
  '별지25': {
    title: '교육시간 관리대장',
    desc: '관련 교육을 근무시간으로 인정받는 경우 작성·보관하는 서식입니다.',
    current: 'https://www.suwonudc.co.kr/swjjc/base/board/read?boardManagementNo=36&boardNo=1931',
    archive2025: 'https://www.carefor.co.kr/cs/view_notice.php?calmgno=45793&rtnUrl=%2Fcs%2Flist_notice.php%3F'
  },
  '별지26': {
    title: '프로그램 운영기록지',
    desc: '프로그램 제공내용 등을 기록하는 서식입니다.',
    current: 'https://www.suwonudc.co.kr/swjjc/base/board/read?boardManagementNo=36&boardNo=1931',
    archive2025: 'https://www.carefor.co.kr/cs/view_notice.php?calmgno=45793&rtnUrl=%2Fcs%2Flist_notice.php%3F'
  }
};
...
function getCommonRows(service, yearValue){
  ...
  const visits = ['방문요양','방문목욕','방문간호'];
  if(year === 2025){
    if(visits.includes(service)){
      return [
        row('자체','배상책임보험 증서'), row('자체','직원 근무일지(출근부)'),
        row('별지24','프로그램관리자·사회복지사 업무수행일지','2025.1.1.~6.30.'),
        row('별지24','가정방문형 프로그램관리자 및 사회복지사 업무수행 일지','2025.7.1.~12.31.','2025.7.1. 서식 개정'),
        row('별지25','교육시간 관리대장')
      ];
    }
    if(service === '주야간보호'){
      return [
        row('자체','배상책임보험 증서'), row('자체','직원 근무일지(출근부)'),
        row('별지22','이동서비스일지'), row('별지23','주·야간보호 목욕서비스 제공일지'),
        row('별지24','프로그램관리자·사회복지사 업무수행일지','2025.1.1.~6.30.'),
        row('별지24-2','주·야간보호 및 치매전담형 프로그램관리자 업무수행 일지','2025.7.1.~12.31.','2025.7.1. 서식 개정'),
        row('별지25','교육시간 관리대장'), row('별지26','프로그램 운영기록지')
      ];
    }
    if(service === '주야간보호 치매전담실'){
      return [
        row('자체','배상책임보험 증서'), row('자체','직원 근무일지(출근부)'),
        row('별지22','이동서비스일지'), row('별지23','주·야간보호 목욕서비스 제공일지'),
        row('별지24-2','치매전담형 프로그램관리자 업무수행일지','2025.1.1.~6.30.'),
        row('별지24-2','주·야간보호 및 치매전담형 프로그램관리자 업무수행 일지','2025.7.1.~12.31.','2025.7.1. 서식 개정'),
        row('별지25','교육시간 관리대장'), row('별지26','프로그램 운영기록지')
      ];
    }
  ...
  if(year >= 2026){
    if(visits.includes(service)){
      return [
        row('자체','배상책임보험 증서'), row('자체','직원 근무일지(출근부)'),
        row('별지24','가정방문급여 급여관리 업무수행일지','2026.1.1.부터','2026년 서식 개편'),
        row('별지25','교육시간 관리대장')
      ];
    }
    if(service === '주야간보호'){
      return [
        row('자체','배상책임보험 증서'), row('자체','직원 근무일지(출근부)'),
        row('별지22','이동서비스일지'), row('별지23','주·야간보호 목욕서비스 제공일지'),
        row('별지24-2','주야간보호기관 인지활동형 프로그램관리자 업무수행 일지','2026.1.1.부터','2026년 서식 분리'),
        row('별지25','교육시간 관리대장'), row('별지26','프로그램 운영기록지')
      ];
    }
    if(service === '주야간보호 치매전담실'){
      return [
        row('자체','배상책임보험 증서'), row('자체','직원 근무일지(출근부)'),
        row('별지22','이동서비스일지'), row('별지23','주·야간보호 목욕서비스 제공일지'),
        row('별지24-3','치매전담형 기관 프로그램관리자 업무수행 일지','2026.1.1.부터','2026.1.1. 신설'),
        row('별지25','교육시간 관리대장'), row('별지26','프로그램 운영기록지')
      ];
    }
    if(service === '노인요양시설 치매전담실'){
...
function formResourceLink(code, yearValue){
  ...
  if(!r) return '';
  const year = parseInt(String(yearValue),10);
  if(['별지24','별지24-2','별지24-3'].includes(code) && r.worklogPdf && year >= 2025) return r.worklogPdf;
  if(year === 2025 && r.archive2025) return r.archive2025;
  return r.current || '';
}
...
$('#makeLabelBtn').addEventListener('click',()=>{
  ...
  const renderCommon = (common, year)=> {
    ...
    const rows=common.map(item=>{
      ...
      const view=(!isSelf && link) ? `<a class="form-view-btn" href="${link}" target="_blank" rel="noopener">서식 보기</a>` : '';
  ...
  const renderYearNotice=(year)=>{
    ...
    if(y===2025) return `<div class="year-revision-alert"><strong>2025년 서식 개정 확인</strong><span>별지 제24호·제24호의2는 2025년 7월 1일 개정되었습니다. 1~6월과 7~12월 자료의 적용 서식을 구분해 확인하세요.</span></div>`;
    if(y>=2026) return `<div class="year-revision-alert current"><strong>2026년 업무수행일지 서식 개편</strong><span>2026년 1월 1일부터 별지24(가정방문급여), 별지24의2(주야간보호), 별지24의3(치매전담형)으로 작성대상이 구분됩니다.</span></div>`;
    return '';
  };
  ...
  const resourceIntro=`<aside class="form-resource-guide"><strong>서식자료도 함께 확인하세요</strong><span>파란색 ‘서식 보기’를 누르면 선택한 연도에 맞는 고시·세부사항 자료 또는 해당 별지서식을 새 창에서 확인할 수 있습니다. 특히 2025년 7월 1일과 2026년 1월 1일 업무수행일지 개정을 구분해 확인하세요.</span></aside>`;
