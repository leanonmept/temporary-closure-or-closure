...
$('#makeLabelBtn').addEventListener('click',()=>{
  ...
  const resourceIntro=`<aside class="form-resource-guide"><strong>각 별지 서식을 바로 확인할 수 있습니다.</strong><span><b>2025·2026년 별지 서식은 사이트 안에서 바로 미리보기</b>할 수 있습니다. 미리보기는 해당 서식 시작 쪽에서 열리고, 여러 쪽 서식은 페이지 범위를 함께 표시합니다. 2021~2024년은 해당 연도 세부사항 원문·첨부자료 페이지로 연결합니다.</span><div class="form-source-links">${selectedSourceLinks}</div></aside>`;
...
document.querySelectorAll('.overview-card, .overview-wide').forEach(link=>{
  link.addEventListener('click', e=>{
    ...
    if(id==='binding' && target){
      ...
      history.replaceState(null,'','#binding');
      scrollToEl(target);
      return;
    }
