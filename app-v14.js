...
$('#makeLabelBtn').addEventListener('click',()=>{
  ...
  const renderCommon = (common)=> {
    ...
    const rows=common.map(([code,name])=>{
      const searchable = code !== '자체';
      return `<li><span class="dot">•</span><span class="form-code ${code==='자체'?'self-code':''}">[${code}]</span><strong>${name}</strong>${searchable?'<span class="search-mark" aria-hidden="true">🔍</span>':''}</li>`;
    }).join('');
