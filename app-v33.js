const TYPES = {
  ...
  lost: {
    ...
    docs: [
      ...
      '확인서 [별지 제2호 서식] — 분실 또는 훼손에 체크하고 일부·전부 구분 및 사유 작성',
      '대리인이 방문하는 경우: 아래 신청인 확인서류를 추가 확인'
    ],
    flows: [
      ...
      ['확인서 작성', '확인서에서 분실 또는 훼손에 체크한 뒤 일부·전부를 구분하고, 해당 자료와 사유를 구체적으로 작성합니다.'],
  ...
  nohistory: {
    ...
    flows: [
      ...
      ['확인서 작성', '확인서의 급여제공내역 없음 항목에 체크하고, 필요한 경우 관련 내용을 작성합니다.'],
...
const FORM_RESOURCES = {
  '별지22': {
    ...
    ranges: {2025:[50,51], 2026:[60,61]}
  },
  '별지23': {
    title: '주·야간보호 목욕서비스 제공 일지',
    desc: '주·야간보호 목욕서비스 제공 시 작성·보관하는 서식입니다.',
    ranges: {2025:[52,52], 2026:[62,62]}
  },
  '별지24': {
    title: '가정방문급여 급여관리 업무수행 일지',
    desc: '2026년부터 가정방문급여(통합재가서비스 포함) 급여관리 업무수행 일지로 개편된 서식입니다.',
    ranges: {2025:[53,55], 2026:[63,65]}
  },
  '별지24-2': {
    title: '주야간보호기관 인지활동형 프로그램관리자 업무수행 일지',
    desc: '2026년부터 주야간보호기관용 업무수행 일지로 구분된 서식입니다.',
    ranges: {2025:[56,58], 2026:[66,68]}
  },
  '별지24-3': {
  ...
  '별지25': {
    ...
    ranges: {2025:[59,59], 2026:[72,72]}
  },
  '별지26': {
    title: '프로그램 운영 기록지',
    desc: '프로그램 제공내용과 평가 등을 작성·보관하는 서식입니다.',
    ranges: {2025:[62,63], 2026:[75,76]}
  }
};
...
function formResourceMeta(code, yearValue){
  const r = FORM_RESOURCES[code];
  if(!r) return null;
  const year = parseInt(String(yearValue), 10);
  const range = r.ranges?.[year];
  if(!range) return null;
  const pdf = year === 2025 ? PDF_2025 : (year >= 2026 ? PDF_2026 : null);
  if(!pdf) return null;
  ...
  return {
    ...
    startPage,
    endPage,
    pageLabel:startPage===endPage?`${startPage}쪽`:`${startPage}~${endPage}쪽`,
    url:`${pdf}#page=${startPage}&zoom=page-width&navpanes=0`,
    rawUrl:`${pdf}#page=${startPage}`
  };
}

function formResourceLink(code, yearValue){
  const meta=formResourceMeta(code,yearValue);
...
function formLinkLabel(yearValue){
  const year = parseInt(String(yearValue), 10);
  return year >= 2025 ? '서식 미리보기' : '연도 서식';
}

function openFormModal(code, yearValue){
  const meta=formResourceMeta(code,yearValue);
  if(!meta){
    const url=formResourceLink(code,yearValue);
    if(url) window.open(url,'_blank','noopener');
    return;
  }
  const modal=document.getElementById('formModal');
  document.getElementById('formModalKicker').textContent=`${meta.year}년 · ${meta.code}`;
  document.getElementById('formModalTitle').textContent=meta.title;
  document.getElementById('formModalMeta').textContent=`공단 세부사항 ${meta.pageLabel} · ${meta.desc}`;
  document.getElementById('formModalOpen').href=meta.rawUrl;
  document.getElementById('formModalFrame').src=meta.url;
...
function closeFormModal(){
  ...
  const frame=document.getElementById('formModalFrame');
...
// 작성 예시는 항상 편철 가이드 바로 위에 배치합니다.
const transferExampleSection=$('#transfer-list-example');
const bindingSection=$('#binding');
if(transferExampleSection && bindingSection && transferExampleSection.nextElementSibling!==bindingSection){
  bindingSection.parentNode.insertBefore(transferExampleSection,bindingSection);
}

function revealGuide(targetId){
  $$('[data-guide-content]').forEach(el=>el.classList.remove('hidden'));
  const target=document.getElementById(targetId);
  if(!target) return;
  history.replaceState(null,'',`#${targetId}`);
  setTimeout(()=>scrollToEl(target),60);
}
...
function renderDocs(t){
  ...
  if(typeKey==='lost'){
    ...
    help.innerHTML=`<strong>확인서는 이렇게 작성하세요.</strong><p>확인서에서 <b>분실 또는 훼손</b>에 체크하고 <b>일부·전부</b>를 구분합니다. 분실한 경우에는 사유를 적고, 훼손된 경우에는 해당 자료와 훼손 내용을 확인할 수 있도록 작성합니다.</p>`;
  }else if(typeKey==='nohistory'){
    help.classList.remove('hidden');
    help.innerHTML=`<strong>확인서는 이렇게 작성하세요.</strong><p>확인서의 <b>급여제공내역 없음</b> 항목에 체크합니다. 분실·훼손이나 급여제공내역 없음에 해당하지 않는 별도 사유가 있으면 <b>기타</b>에 체크하고 내용을 작성합니다.</p>`;
  }else if(typeKey==='self'){
    help.classList.remove('hidden');
...
function checkItems(){
  ...
  if(typeKey!=='nohistory'){
    extras.push('휴·폐업 기준월을 포함한 총 61개월분의 이관 대상자료를 확인했습니다.');
  }
  if(typeKey==='transfer'){
  ...
  if(typeKey==='lost'){
    ...
    extras.push('확인서에서 분실·훼손의 일부·전부를 구분하고 해당 사유와 내용을 작성했습니다.');
    extras.push('과태료가 부과될 수 있다는 안내를 확인했습니다.');
  }
  if(typeKey==='nohistory'){
    extras.push('이관 목록표를 작성했습니다.');
    extras.push('확인서의 급여제공내역 없음 항목에 체크했습니다.');
...
$$('[data-jump]').forEach(btn=>btn.addEventListener('click',()=>{
  if(btn.dataset.jump==='binding') return revealGuide('binding');
  scrollToEl(document.getElementById(btn.dataset.jump));
}));
...
$('#resetBtn').addEventListener('click',()=>{history.replaceState(null,'',location.pathname+location.search);$('.faq-list details').forEach(el=>el.open=false);business=null;typeKey=null;operation=null;resetApplicantChoice();$$('[data-business], [data-operation]').forEach(b=>b.classList.remove('selected'));$('#typeSection').classList.add('hidden');$('#operationSection').classList.add('hidden');$('#resultSection').classList.add('hidden');$$('[data-content]').forEach(el=>el.classList.add('hidden'));scrollToEl($('#start'));});
...
$('#makeLabelBtn').addEventListener('click',()=>{
  ...
  const renderCommon = (common, year)=> {
    ...
    const rows=common.map(item=>{
      ...
      const hasInlinePreview=(!isSelf && FORM_RESOURCES[code] && yearNum>=2025 && formResourceMeta(code,year));
      const view=(!isSelf && link) ? (hasInlinePreview
        ? `<button type="button" class="form-view-btn" data-form-preview data-form-code="${code}" data-form-year="${yearNum}">${formLinkLabel(year)}</button>`
        : `<a class="form-view-btn" href="${link}" target="_blank" rel="noopener">${formLinkLabel(year)}</a>`) : '';
  ...
  $('#labelResult').innerHTML=yearCards;
...
// 편철 가이드는 상황 선택과 무관하게 바로 열 수 있습니다.
// 그 밖의 업무별 안내는 기관 상황을 먼저 선택해야 정확한 내용을 보여 줍니다.
document.querySelectorAll('.overview-card, .overview-wide').forEach(link=>{
  link.addEventListener('click', e=>{
    const id=(link.getAttribute('href')||'').replace('#','');
    const target=document.getElementById(id);
    if(id==='binding' && target){
      e.preventDefault();
      revealGuide('binding');
      return;
    }
...
// 자료이관 영역 안의 편철 가이드 링크도 같은 방식으로 바로 이동합니다.
document.querySelectorAll('a[href="#binding"]').forEach(link=>{
  if(link.matches('.overview-card, .overview-wide')) return;
  link.addEventListener('click', e=>{
...
// 질문 한눈에 보기에서 선택한 답변을 열고 정확한 위치로 이동합니다.
document.querySelectorAll('.question-overview-card').forEach(link=>{
  link.addEventListener('click', e=>{
    const id=(link.getAttribute('href')||'').replace('#','');
    const target=document.getElementById(id);
    if(!target) return;
    e.preventDefault();
    if(id==='transfer-list-example'){
      revealGuide('transfer-list-example');
      return;
    }
...
// 편철가이드 서식 미리보기: 공식 PDF를 사이트 안의 큰 화면으로 확인합니다.
document.addEventListener('click', e=>{
  const btn=e.target.closest('[data-form-preview]');
  if(btn){
    openFormModal(btn.dataset.formCode, btn.dataset.formYear);
    return;
  }
  if(e.target.closest('[data-form-close]')) closeFormModal();
});
document.addEventListener('keydown', e=>{ if(e.key==='Escape') closeFormModal(); });

const initialGuideTarget=location.hash.replace('#','');
if(initialGuideTarget==='binding' || initialGuideTarget==='transfer-list-example'){
  revealGuide(initialGuideTarget);
}
